...
$doc
  ...
  .on 'loadScore', (e, startDate, endDate, scoreItem) ->
    if not startDate then startDate = getRangeByDays(7)[0]
    if not endDate then endDate = getRangeByDays(7)[1]
    data.getScore($doc.data('userId'), startDate, endDate).then (result) ->
      ...
      otherscoreTotal = Math.round((result.workloadSum / result.workloadCount + result.efficiencySum / result.efficiencyCount + result.cooperativeSum / result.cooperativeCount) * 10) / 10
      ...
      else filterScore = (scoreList) ->
        vals = _.map scoreObj, (val, key) -> val
        total = _.r educe vals, ((memo, num) -> memo + num), 0
    ...
    $(dlg.node)
      ...
      .on 'click', '.btn-ok', ->
        ...
        if uid and workload and efficiency and cooperative
          data.markUser(uid, {workload, efficiency, cooperative}).then () ->
            null
          .fail ->
            alert '请求失败'
