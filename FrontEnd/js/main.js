...
(function() {
  var $doc, currentUser, getRangeByDays, myUserID;
  ...
  $doc.on('init', function() {
    var $dayindex, $dayslider, $itemSelector;
    ...
    return $itemSelector = data.getUser().then(function(user) {
      myUserID = user.id;
    ...
    if (!startDate) {
      startDate = getRangeByDays(7)[0];
    }
    if (!endDate) {
      endDate = getRangeByDays(7)[1];
    }
    return data.getScore($doc.data('userId'), startDate, endDate).then(function(result) {
      ...
      $doc.find('.score').find('.timerange').find('.daterange').html(startDate + '-' + endDate).end().find('.daycount').html(result.dayCount).end().end().find('.statusicon').removeClass('current').filter('.statusicon-' + status).addClass('current').end().end().end().find('.radialchart-text').find('.otherscore').html(otherscoreTotal).end().end();
      rWidth = 142;
      rHeight = 142;
      rMargin = {
        top: 5,
      ...
      arcPoint = getRadialChartChild().selectAll('circle').data(averageScores).enter().append('circle').attr('r', 6).attr('transform', function(d, i) {
        ...
        d3.select(this).transition().duration(1000).attr('fill', function() {
      ...
      if (barchartSelector) {
        ...
        filterScore = function(scoreList) {
          var total, vals;
          vals = _.map(scoreObj, function(val, key) {
            return val;
          });
          total = _.r(educe(vals, (function(memo, num) {
            return memo + num;
          }), 0));
          if (vals && vals.length) {
    ...
    $(dlg.node).find('.user-search').autocomplete({
      ...
      _ref = _.map(['workload', 'efficiency', 'cooperative'], function(item) {}), workload = _ref.workload, efficiency = _ref.efficiency, cooperative = _ref.cooperative;
